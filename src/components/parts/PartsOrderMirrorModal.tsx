    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/80 backdrop-blur-sm flex items-center justify-center p-3 sm:p-6 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-[#121215] border border-neutral-200 dark:border-[#27272a] rounded-2xl w-full max-w-4xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh] text-neutral-900 dark:text-white">
        ...
        <div ref={printRef} className="flex-1 overflow-y-auto p-5 sm:p-7 space-y-6 text-neutral-200 bg-[#0f0f12]">
          ...
          {/* Printable Header */}
          <div className="border-b border-neutral-200 dark:border-neutral-800 pb-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
            ...
            <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-3 text-right">
              <div className="text-[10px] text-neutral-500 dark:text-neutral-400 uppercase font-mono font-bold">Número do Pedido</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white dark:bg-neutral-900/70 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-4 space-y-2">
          ...
          <div className="bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 text-xs">
              <div className="p-2.5 rounded-xl bg-white dark:bg-neutral-900/80 border border-neutral-200 dark:border-neutral-800">
                <span className="text-[10px] text-neutral-500 dark:text-neutral-400 block font-bold">1. Transmissão</span>
                <span className="text-emerald-400 font-bold flex items-center gap-1 mt-0.5">
                  <CheckCircle2 className="w-3.5 h-3.5" /> Transmitido
                </span>
                <span className="text-[10px] text-neutral-500 dark:text-neutral-400 block mt-0.5">{order.createdAt}</span>
              </div>

              <div className="p-2.5 rounded-xl bg-white dark:bg-neutral-900/80 border border-neutral-200 dark:border-neutral-800">
                <span className="text-[10px] text-neutral-500 dark:text-neutral-400 block font-bold">2. Estoque CD</span>
                {order.stockVerified ? (
                  <span className="text-emerald-400 font-bold flex items-center gap-1 mt-0.5">
              ...
              <div className="p-2.5 rounded-xl bg-white dark:bg-neutral-900/80 border border-neutral-200 dark:border-neutral-800">
                <span className="text-[10px] text-neutral-500 dark:text-neutral-400 block font-bold">3. Crédito & Margem</span>
              ...
              <div className="p-2.5 rounded-xl bg-white dark:bg-neutral-900/80 border border-neutral-200 dark:border-neutral-800">
                <span className="text-[10px] text-neutral-500 dark:text-neutral-400 block font-bold">4. ERP TOTVS Protheus</span>
                {order.protheusIntegrated ? (
                  <span className="text-emerald-400 font-bold flex items-center gap-1 mt-0.5">
                    <Database className="w-3.5 h-3.5" /> Integrado SC5
                  </span>
                ) : (
                  <span className="text-neutral-500 dark:text-neutral-400 font-bold flex items-center gap-1 mt-0.5">
                    <Clock className="w-3.5 h-3.5" /> Pendente
                  </span>
                )}
                <span className="text-[10px] text-neutral-500 dark:text-neutral-400 block mt-0.5 font-mono">
              ...
              <div className="p-3 rounded-xl bg-blue-950/40 border border-blue-800/50 flex flex-wrap items-center justify-between gap-3 text-xs">
                <div>
                  ...
                  {order.protheusNFeNumber && (
                    <span className="text-neutral-700 dark:text-neutral-300 ml-3 font-mono">| {order.protheusNFeNumber}</span>
                  )}
                </div>
          ...
          <div className="space-y-2">
            ...
            <div className="border border-neutral-200 dark:border-neutral-800 rounded-2xl overflow-hidden">
              <table className="w-full text-left text-xs border-collapse font-tabular">
                <thead className="bg-white dark:bg-neutral-900 text-neutral-500 dark:text-neutral-400 uppercase text-[10px] font-bold border-b border-neutral-200 dark:border-neutral-800">
                  <tr>
                ...
                <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
                  {order.items.map((item, idx) => (
                    <tr key={item.id} className="hover:bg-white dark:bg-neutral-900/40">
                      <td className="p-3 pl-4 font-mono font-bold text-white">
                        <div className="flex items-center gap-1.5">
                          <span className="text-neutral-500 dark:text-neutral-400 text-[10px]">#{idx + 1}</span>
                      ...
                      <td className="p-3">
                        ...
                        <div className="text-[11px] text-neutral-500 dark:text-neutral-400">{item.modelName}</div>
                      </td>
                      ...
                      <td className="p-3 text-right font-mono text-neutral-500 dark:text-neutral-400">
          ...
          {/* Financial Totals */}
          <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="text-xs text-neutral-500 dark:text-neutral-400 space-y-1">
              <div>Condição Comercial: <span className="text-white font-bold">{order.paymentMethod}</span></div>
              {order.notes && <div>Observações: <span className="text-neutral-700 dark:text-neutral-300">{order.notes}</span></div>}
            </div>
            ...
            <div className="text-right space-y-1 self-end sm:self-auto font-tabular">
              ...
              <div className="text-base sm:text-lg font-black text-amber-400 font-mono pt-1 border-t border-neutral-200 dark:border-neutral-800">
          ...
          {isMontadora && (
            <div className="bg-white dark:bg-neutral-900 border border-neutral-700 p-4 rounded-2xl space-y-3 print:hidden">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-neutral-500 dark:text-neutral-400">Ambiente de Operações & ERP</span>
              </div>
